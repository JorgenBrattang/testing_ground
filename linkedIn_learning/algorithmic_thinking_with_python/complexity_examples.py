...
# The smaller, constant time operation (O(1)) is ignored.
count = 0  # Assignment statement takes O(1) time.
while count < n:
    count += 1

# If the for loop counter increases or decreases
# by a constant, the cost is O(n).
...
#  If a loop counter increases or decreases by a
# multiple, the cost is O(log(n)).
n = 100
while n > 0:
    n = n // 2
